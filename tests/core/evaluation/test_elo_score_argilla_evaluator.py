...
def test_evaluate_run_submits_pairwise_comparison_records(
    ...
    instruction_completion = "Whatever"
    instruct_output = PromptOutput(
        response=CompletionResponse(
            model_version="",
            completions=[CompletionResult(completion=instruction_completion)],
    ...
    in_memory_evaluation_repository.store_example_output(
        example_output=ExampleOutput(
            run_id=run_id1, example_id="example_id", output=instruct_output
        )
    )
    in_memory_evaluation_repository.store_example_output(
        example_output=ExampleOutput(
            run_id=run_id2, example_id="example_id", output=instruct_output
        )
    )
    ...
    evaluation_overview = evaluator.evaluate_runs(run_id1, run_id2)
    ...
    assert argilla_fake.record_data(evaluation_overview.id) == [
        RecordData(
            content={
                ...
                "response1": instruction_completion,
                "response2": instruction_completion,
            },
            example_id=example_id,
        )
    ]
