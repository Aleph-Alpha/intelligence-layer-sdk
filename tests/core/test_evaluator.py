...
class TestTaskEvaluator(Evaluator[None, None, TestEvaluation, None]):
    def do_evaluate(
        self, input: None, tracer: Tracer, expected_output: None
    ) -> TestEvaluation:
        raise RuntimeError("Test Exception")

    def aggregate(self, evaluations: Iterable[TestEvaluation]) -> None:
        return None


def test_evaluate_dataset_does_not_raise_exception_on_failed_example() -> None:
    evaluation_repository = InMemoryEvaluationRepository()
    evaluator = TestTaskEvaluator(evaluation_repository)
    ...
    results = evaluation_repository.evaluation_run_results(
        evaluation_run_overview.id, TestEvaluation
    )
