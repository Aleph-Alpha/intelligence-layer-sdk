...
from intelligence_layer.core import (IntelligenceApp, InvalidTaskError)
...
class TaskWithTaskSpanAsInput(Task[None, None]):
...
def test_serve_task_can_serve_multiple_tasks(intelligence_app: IntelligenceApp) -> None:
    client = TestClient(intelligence_app.fast_api_app)
    path = "/path"
    path2 = "/path2"
...
def test_serve_task_throws_error_if_task_untyped(intelligence_app: IntelligenceApp) -> None:
    path = "/path"
    
    with raises(InvalidTaskError) as error:
        intelligence_app.register_task(UntypedTask(), path)

    assert error.value.message == "The task `do_run` method needs a type for its input, task_span and return value."

def test_serve_task_throws_error_if_no_task_span_type(intelligence_app: IntelligenceApp) -> None:
    path = "/path"
    
    with raises(InvalidTaskError) as error:
        intelligence_app.register_task(TaskWithoutTaskSpanType(), path)

    assert error.value.message == "The task `do_run` method needs a `TaskSpan` type as its second argument."

def test_serve_task_throws_error_if_input_is_taskspan(intelligence_app: IntelligenceApp) -> None:
    path = "/path"

    with raises(InvalidTaskError) as error:
        intelligence_app.register_task(TaskWithTaskSpanAsInput(), path)

    assert error.value.message == "The task `do_run` method cannot have a `TaskSpan` type as input."
