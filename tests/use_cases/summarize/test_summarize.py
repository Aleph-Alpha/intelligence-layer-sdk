...
from intelligence_layer.use_cases.summarize.base.base_summarize import SummarizeInput
from intelligence_layer.use_cases.summarize.medium_compression.medium_compression_summarize import (
    MediumCompressionSummarize,
)


@fixture
def summarize(client: Client) -> MediumCompressionSummarize:
    return MediumCompressionSummarize(client)


def test_summarize(summarize: MediumCompressionSummarize) -> None:
    text = Chunk(
    ...
    input = SummarizeInput(chunk=text, use_highlights=False)
