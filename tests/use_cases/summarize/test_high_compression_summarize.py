...
@fixture
def high_compression_summarize(client: Client) -> HighCompressionSummarize:
    return HighCompressionSummarize(client)
...
def test_high_compression_summarize_is_language_sensitive(
    ...
    input_en = SummarizeInput(chunk=text, language=Language("en"))
    input_de = SummarizeInput(chunk=text, language=Language("de"))
