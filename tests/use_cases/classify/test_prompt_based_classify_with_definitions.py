...
from intelligence_layer.core import NoOpTracer, TextChunk, LuminousControlModel
...
def labels_with_definitions() -> Sequence[LabelWithDefinition]:
    return [
        LabelWithDefinition(
            name="Dinosaur",
            definition="Any text that is about dinosaurs."
        ),
        LabelWithDefinition(
            name="Plant",
            definition="Any text that is about plants."
        ),
        LabelWithDefinition(
            name="Toy",
            definition="Everything that has something to do with toys."
        )
    ]
...
def test_prompt_based_classify_with_definitions_returns_score_for_all_labels(
    ...
    classify_output = prompt_based_classify_with_definitions.run(classify_input, NoOpTracer())
