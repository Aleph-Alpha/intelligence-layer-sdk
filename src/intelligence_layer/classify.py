...
def to_aa_tokens(tokens: Sequence[Token]) -> AATokens:
    return AATokens.from_token_ids([token.token_id for token in tokens])
...
### Response:{{label}}"""
    ...
    def run(self, input: ClassifyInput) -> ClassifyOutput:
        ...
        normalized_probs_per_label = self._normalize(log_probs_per_label)
    ...
    def _normalize(
        self, log_probs_per_label: Mapping[str, Sequence[TokenWithProb]]
    ...
    def _complete_per_label(
        ...
        debug_log: DebugLog,
    ) -> Mapping[str, CompletionResponse]:
        debug_log.add(
            "Completion",
            {
                "model": model,
        ...
        return {
            label: self._complete(
                ...
                text=text,
                label=to_aa_tokens(tokens),
            )
            for label, tokens in tokenized_labels.items()
        }

    def _complete(
        self, model: str, prompt_template: PromptTemplate, **kwargs: Any
    ) -> CompletionResponse:
    ...
    def _get_log_probs_of_labels(
        self,
        completion_responses: Mapping[str, CompletionResponse],
        tokenized_labels: Mapping[str, Sequence[Token]],
        debug_log: DebugLog,
    ) -> Mapping[str, Sequence[TokenWithProb]]:
        logs_probs_per_label = {
            label: self._get_log_probs_of_label(completion_responses[label], tokens)
            for label, tokens in tokenized_labels.items()
        }
        pprint(logs_probs_per_label)
    ...
    def _get_log_probs_of_label(
        ...
        return [
            TokenWithProb(
                token=token, prob=LogProb(log_prob_dict.get(token.token, 0.0) or 0.0)
            )
            for token, log_prob_dict in zip(tokens, log_prob_dicts)
        ]
...
class TreeNode:
    ...
    def path(self, tokens: Iterable[Token]) -> Iterable[TokenWithProb]:
        ...
        for token in tokens:
            ...
            assert child
            node = child
            assert node.token and node.prob
            yield TokenWithProb(token=node.token, prob=node.prob)
