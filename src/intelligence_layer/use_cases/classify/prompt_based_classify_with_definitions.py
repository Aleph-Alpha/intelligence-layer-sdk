...
from intelligence_layer.core import CompleteInput, CompleteOutput, ControlModel, LuminousControlModel, Task, TaskSpan, TextChunk
...
class PromptBasedClassifyWithDefinitions(Task[ClassifyInput, SingleLabelClassifyOutput]):
...
Reply with only the class label."""
    ...
    def do_run(self, input: ClassifyInput, task_span: TaskSpan) -> SingleLabelClassifyOutput:
        ...
        return SingleLabelClassifyOutput(
            scores=self._build_scores(complete_output)
        )

    def _get_prompt(self, chunk: TextChunk, labels: frozenset[str]) -> Prompt:
        def format_input(text: str, labels: frozenset[str]) -> str:
            definitions = '\n'.join(label.to_string() for label in self._labels_with_definitions if label.name in labels)
            return f"""Text: {text}
...
{definitions}"""
        ...
        unexpected_labels = labels - set(label.name for label in self._labels_with_definitions)
