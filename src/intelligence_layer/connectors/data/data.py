...
from intelligence_layer.connectors.data.models import DataRepository, DataRepositoryCreate, Dataset, DatasetCreate
from intelligence_layer.connectors.data.exceptions import DataInternalError, ExternalServiceUnavailable, ForbiddenError, InvalidInput, ResourceNotFound
...
class DataClient:
    ...
    def list_repositories(self, page: int = 0, size: int = 20) -> List[DataRepository]:
        ...
        return [DataRepository(**repository) for repository in repositories["repositories"]]
    ...
    def get_repository(self, repository_id: str) -> DataRepository:
        ...
        """
        url = urljoin(self.base_data_platform_url, f"api/v1/repositories/{repository_id}")
    ...
    def create_dataset(self, repository_id: str, dataset: DatasetCreate) -> Dataset:
        ...
        Returns:
            Dataset new object
        """
        url = urljoin(self.base_data_platform_url, f"api/v1/repositories/{repository_id}/datasets")
        response = self._do_request(
            "POST",
            url,
            files={"source_data": dataset.source_data, "labels": ",".join(dataset.labels), "total_units": dataset.total_units}
        )
        return Dataset(**response.json())

    def list_datasets(self, repository_id: str, page: int = 0, size: int = 20) -> List[Dataset]:
        """
    ...
    def get_dataset(self, repository_id: str, dataset_id: str) -> Dataset:
        ...
        Dataset new entity
        """
        url = urljoin(self.base_data_platform_url, f"api/v1/repositories/{repository_id}/datasets/{dataset_id}")
        response = self._do_request("GET", url)
        return Dataset(**response.json())
    ...
    def stream_dataset(self, repository_id: str, dataset_id: str) -> Iterator[Any]:
        ...
        """
        url = urljoin(self.base_data_platform_url, f"api/v1/repositories/{repository_id}/datasets/{dataset_id}/datapoints")
