...
class HybridQdrantInMemoryRetriever(QdrantInMemoryRetriever):
    ...
    def get_relevant_documents_with_scores(
        self, query: str, filter: Optional[models.Filter] = None
    ) -> Sequence[SearchResult[int]]:
        """Search for documents in memory using hybrid search.

        This method first embeds the given query using both a dense and sparse embedding model.
        Then, it performs two search requests, one for each embedding type.
        The results are then fused using the Reciprocal Rank Fusion algorithm.

        Args:
            query: The text to be searched with.
            filter: Optional conditions to filter by.
    ...
    def get_filtered_documents_with_scores(
        ...
        Returns:
            All documents that correspond to the query and pass the filter.
        """
        return self.get_relevant_documents_with_scores(query, filter)
